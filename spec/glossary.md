...
| D4 instance | `Instance` 记录 | `fractal_kernel`（Phase 2） |
...
| D6 g/v/r 判定点 | `g(c)` / `v(c,e)` / `r(c,e,p)` | scheduler + llm（Phase 1–2） |
| D8 task/attempt/checkpoint | `Task` / `Attempt` / `Checkpoint` | `fractal_kernel`（Phase 2） |
| D12 guard | Guard 规则集 | `fractal_kernel/guard.py` |
...
| 事件日志 | EventLog | `fractal_kernel/store.py`（ADR-0003） |
| D7 委托深度 | `d(i) ≤ D_max` | `fractal_kernel/dag.py` |
| D5.5 policy | `Policy` | `fractal_kernel`（Phase 1） |

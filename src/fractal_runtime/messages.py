...
Ported from huggingface/tau ``tau_agent/messages.py`` v0.4.1 (MIT).

Deviations from the source (ADR-0006 port log, docs/dev-notes/phase-1-agent-loop-port.md):
session/UI-only message roles are excluded — BashExecutionMessage,
CustomMessage, BranchSummaryMessage, CompactionSummaryMessage — so
``AgentMessage`` is the three-role union the headless build loop needs.
``message_to_user`` / ``message_text`` (session rendering helpers) dropped.
"""
